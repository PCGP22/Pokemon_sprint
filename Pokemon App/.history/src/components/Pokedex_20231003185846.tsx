...
function Pokedex(props: PokedexProps) {
  ...
  function handleSearch() {
    ...
    let result = allData.filter((d: any) => d.name.includes(searchTerm));
    setId(result[0].id);
  }
    ...
    <div>
      <main>
        ...
        <section>
          ...
          <progress value={data.stats[0].base_stat} max="100" /> <br />
          <strong>Attack:</strong>
          <span>{data.stats[1].base_stat}</span>
          <progress value={data.stats[1].base_stat} max="100" /> <br />
          <strong>Defense:</strong>
          <span>{data.stats[2].base_stat}</span>
          <progress value={data.stats[2].base_stat} max="100" /> <br />
          <strong>Sp-att:</strong>
          <span>{data.stats[3].base_stat}</span>
          <progress value={data.stats[3].base_stat} max="100" /> <br />
          <strong>Sp-def:</strong>
          <span>{data.stats[4].base_stat}</span>
          <progress value={data.stats[4].base_stat} max="100" /> <br />
          <strong>Speed:</strong>
          <span>{data.stats[5].base_stat}</span>
          <progress value={data.stats[5].base_stat} max="100" /> <br />
        </section>
      </main>
    </div>
  );
