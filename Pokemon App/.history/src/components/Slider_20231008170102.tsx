...
function Slider(props: sliderProps) {
  ...
  function handleResize(e: any) {
    ...
    if (e.clientX > 320) {
      setWidth(e.clientX - 325);
    } else if (e.clientX > 640) {
      setWidth(maxWidth);
    } else if (e.clientX === 0) {
      if (value > 50) setWidth(maxWidth);
      if (value < 50) setWidth(0);
  ...
  function handleClick(e: any) {
    console.log(e.clientX);
    if (e.clientX !== 0) {
      setWidth(e.clientX - 25 - 325);
    } else if (e.clientX > maxWidth - 1) {
      setWidth(maxWidth);
    }
    value = Math.trunc(((e.clientX - 25 - 325) / maxWidth) * 10 * 15);
    if (value > 150) value = 150;
    if (value < 50) color = "#f07e1d";
    if (value >= 50) color = "#f0f056";
    if (value > 75) color = "#44db4e";
